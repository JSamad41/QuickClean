...
namespace QuickClean.Models
{
	public class User
	{
		...
		public bool DoesUserLike(Like.Types LikeType, long PropertyID){
            try{
				foreach (Like l in this.Likes){
					if (l.Type == LikeType && l.ID == PropertyID) return true;
                }
				return false;
            }
		...
		public byte GetUserRating(Rating.Types RatingType, long PropertyID)
		{
			try{
				foreach (Rating r in this.Ratings){
					if (r.Type == RatingType && r.ID == PropertyID) return r.Rate;
				}
				return 0;
			}
		...
		public List<Property> GetProperties(long ID = 0){
            try{
				Database db = new Database();
				return db.GetProperties(ID, this.UID);
            }
